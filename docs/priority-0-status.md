...
| 4A. Adaptive interface localization / scale independence | **Partial — immediate priority** | `screen_watcher/layout.py` can already find known title text, infer chat coarsely from timestamps, cache a layout fingerprint and detect layout changes. It is intentionally only ~24 px precise and profiles still resolve detector regions from fixed pixel geometry. Priority 0A must add detector-grade panel boxes, in-game scale estimation, confidence/semantic validation, panel-local child regions, dynamic chat/inventory geometry, temporal reacquisition and multi-resolution replay coverage before profiles are treated as portable between players. |
...
| 6. Layered OCR | **Complete for current numeric path** | RuneScape numeric/sprite OCR is used where applicable with Tesseract fallback. General chat OCR remains the dominant poll-cycle cost, now measured rather than asserted: 570 ms of a ~874 ms four-region cycle, against a ~72 ms process-startup floor that every call pays regardless of image size. Tesseract flag tuning was benchmarked and rejected - `--oem 1` saved 2%, faster page-segmentation modes changed the output, and legacy `--oem 0` was 4.5x slower. The shipped `ocr_scrolling` path is what carries the win: 123 ms against 582 ms on real pixels, now guarded by a timing test that runs the real binary. The numeric regions now cache per cycle like the chat region does - `low_health`, `low_prayer`, `prayer_out` and `adrenaline_full` all read the vitals row, which was OCR'd four times a cycle over identical pixels: 585 ms against 151 ms cached, 435 ms saved. Batching is also IMPLEMENTED as `ocr_many` - one tesseract process for several regions via an `imagelist`, measured at 28% (876 ms -> 630 ms) with byte-identical output. It is not yet wired into the watch loop, which still needs the scheduler to own the cycle (step 2); `FrameScheduler.prefetch` already receives the region list that makes it possible. Native XShm capture was measured and rejected for now: capture is 29.6 ms of a ~874 ms cycle (3.4%), so shared memory would optimise the part that is already negligible. See [engineering notes](engineering-notes.md). |
